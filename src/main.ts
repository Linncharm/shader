...
async function main() {
  console.log('🎨 Shader 学习项目启动! (Three.js + GLSL文件版本)')
  console.log('阶段2和阶段3的所有demo已完成，使用外部.glsl文件')
  
  try {
    // 启动Demo 1: 颜色渐变
    const canvas1 = document.getElementById('canvas1') as HTMLCanvasElement
    if (canvas1) {
      await startDemo1ThreeJS(canvas1)
    } else {
    ...
    if (canvas2) {
      await startDemo2TimeAnimation(canvas2)
    } else {
    ...
    if (canvas3) {
      await startDemo3SDFShapes(canvas3)
    } else {
    ...
    if (canvas4) {
      await startDemo4WaveEffects(canvas4)
    } else {
    ...
    if (canvas5) {
      await startDemo5AdvancedSDF(canvas5)
    } else {
      console.error('Canvas5 not found!')
    }
    
    console.log('✅ 所有shader demo加载完成!')
  } catch (error) {
    console.error('❌ 加载shader文件时出错:', error)
  }
}
